...
void psi_to_share_test(bool mode_switch, size_t batch_size, std::ostream *out)
{
    //-----------数据准备--------- --------------
    ...
    *out << "get_newid: Gen psi_client_response time:" << psi_client_response_time << " ms "
         << std::endl;
    // *out <<"get_newid: psi_server_return_answer time:" << psi_server_return_answer_time << " ms "
    // << std::endl; *out <<"get_newid: psi_client_return_answer time:" <<
    // psi_client_return_answer_time << " ms " << std::endl;
    ...
    *out << "get_newid query size: " << psi_query.byte_size() / 1024.0 << " KBytes" << std::endl;
    ...
    *out << "batch pir to share Query size: " << query.str().size() / 1024.0 << " KBytes"
         << std::endl;
    ...
    *out << "comm. from server to client: "
         << (psi_response.byte_size() + response.str().size()) / 1024.0 << " KBytes" << std::endl;
...
int main(int argc, char *argv[])
{
    ...
    if (argc == 6) {
        ...
        *out << "party_id=" << party_id << std::endl;
        ...
        std::fstream fs;
        if (argv[6][0] != '\0') {
            fs.open(argv[6], std::ios::out | std::ios::app);
            if (fs.is_open()) {
                out = &fs; // 指向文件流
            } else {
                std::cerr << "Failed to open file: " << argv[6] << std::endl;
                return 1;
            }
        }
        *out << "host_log_n_data=" << host_log_n_data << std::endl;
        ...
        *out << "feature_num=" << feature_num << std::endl;
        *out << "party_id=" << party_id << std::endl;
        *out << "intersection ratio=" << inter_ratio << std::endl;
        // psi_to_share_2party(modswitch, host_log_n_data, batch_size, feature_num, party_id);
        psi_to_share_2party_socketopt(
            modswitch, host_log_n_data, batch_size, feature_num, party_id, inter_ratio, out);
    } else {
        // psi_to_share_test(modswitch, 1024, 1);
        psi_to_share_test(modswitch, 1024, &std::cout);
    }
    return 0;
}