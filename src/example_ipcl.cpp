...
int main()
{
    ...
    std::mt19937 rng(dev());
    std::uniform_int_distribution<std::mt19937::result_type> dist(0, UINT_MAX >> 16);
    ...
    std::cout << "Test (x + y) == dec(enc(x) + enc(y)) -- " << (verify ? "pass" : "fail")
              << std::endl
              << std::endl;
    ...
    std::cout << "Test (x + y) == dec(enc(x) + y) -- " << (verify ? "pass" : "fail") << std::endl
              << std::endl;
