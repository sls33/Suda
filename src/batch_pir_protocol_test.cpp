...
void batch_pir_poly_reduce_modswitch_test(bool modswitch)
{
    ...
    // batch_pir_from_nttform_test(isHost, isGuest, result, feature_num, host_log_n_data,
    // batch_size);
    batch_pir_from_nttform_modswitch(
        isHost,
        isGuest,
        data,
        new_ids,
...
void batch_pir_test(
    size_t host_log_n_data, size_t batch_size, size_t feature_num, std::ostream *out)
{
    //-----------数据准备-----------------------
...
void batch_pir_test_1024(std::ostream *out)
{
    //-----------数据准备-----------------------
    size_t feature_num = 8000 / 60;
    size_t host_log_n_data = 20;
...
void batch_pir_test_2048(std::ostream *out)
{
    //-----------数据准备-----------------------
...
int main(int argc, char *argv[])
{
    if (argc < 4) {
        ...
        std::fstream fs;
        if (argv[1][0] != '\0') {
            fs.open(argv[1], std::ios::out | std::ios::app);
            if (fs.is_open()) {
                out = &fs; // 指向文件流
            } else {
        ...
        *out << "payload_bytes=" << payload_bytes << std::endl;
        size_t feature_num = (payload_bytes + 49) / 50 * 8;
        batch_pir_test(host_log_n_data, batch_size, feature_num, out);
    }
