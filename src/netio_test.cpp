...
void test_ECPoint()
{
    ...
    // auto futurePA = std::async(static_cast<void(*)(ECPoint &, std::string&)>(send<ECPoint>),
    // std::ref(ec_point), addres);
    auto futurePB = std::async(recv<ECPoint>, std::ref(result), addres);
    // auto futurePA = std::async(static_cast<void(*)(ECPoint &, std::string&)>(recv<ECPoint>),
    // addres);
