...
void batch_pir_to_share_from_nttform_modswitch(
    bool isHost,
    bool isGuest,
    std::vector<int64_t> &data,
    vector<int64_t> &new_ids,
    vector<vector<int64_t>> &result0,
    vector<vector<int64_t>> &result1,
    ...
    bool modswitch = false)
{
    ...
    if (isHost) {
        ...
        for (size_t feature_id = 0; feature_id < feature_num; feature_id++) {
            ...
            //  std::cout<<__FILE__<<":"<<__LINE__<<std::endl;
            result0[feature_id] =
                poly_host->poly_cipher_toShare(reduced_poly_cipher, poly_chiper_share1[feature_id]);
        }
        end = time(NULL);
...
void load_plaintexts(
    const SEALContext &context, std::vector<Plaintext> &vp, std::ifstream &i_stream)
{
    for (size_t i = 0; i < vp.size(); i++) {
...
void load_plaintexts(
    const SEALContext &context, std::vector<std::vector<Plaintext>> &vvp, std::ifstream &i_stream)
{
    for (size_t i = 0; i < vvp.size(); i++) {
...
public:
    BatchPirToShareServer(
        ...
        : feature_num(feature_num), use_out_mem(use_out_mem)
    {
        ...
        if (batch_size == poly_host->half_poly_mod_degree) {
            ...
            for (size_t i = 0; i < feature_num; i++) {
                pieces_f[i % fake_feature_num] =
                    poly_host->encode(data[i % fake_feature_num], batch_size, false);
            ...
            for (size_t i = 0; i < feature_num / factor; i++) {
                data_tobe_encode = vector<vector<int64_t>>(
                    data.begin() + i % (fake_feature_num / factor) * factor,
                    data.begin() + (i % (fake_feature_num / factor) + 1) * factor);
                ...
                pieces_f[i % (fake_feature_num / factor)] =
                    poly_host->encode(data_tobe_encode, false);
                if (use_out_mem &&
                    (i % (fake_feature_num / factor) == fake_feature_num / factor - 1)) {
                    save_plaintexts(pieces_f, o_stream);
                }
            }
            // std::cout<<__FILE__<<":"<<__LINE__<<endl;
    ...
    stringstream gen_response(BatchPirToShareQuery &query, bool mod_switch = false)
    {
        ...
        for (size_t feature_id = 0; feature_id < feature_num / factor; feature_id++) {
            ...
            // reduced_poly_cipher = poly_host->poly_reduce(pieces_f[feature_id%fake_feature_num],
            // mod_switch);
            reduced_poly_cipher[feature_id] = poly_host->poly_reduce(
                pieces_f[feature_id % (fake_feature_num / factor)], mod_switch);

            // std::cout<<__FILE__<<":"<<__LINE__<<std::endl;
            if (factor == 1) {
                result0[feature_id] = poly_host->poly_cipher_toShare(
                    reduced_poly_cipher[feature_id], poly_chiper_share1[feature_id]);
            } else {
                // std::cout<<__FILE__<<":"<<__LINE__<<std::endl;
                vector<vector<int64_t>> result0_tmp = poly_host->poly_cipher_toShare(
                    reduced_poly_cipher[feature_id], poly_chiper_share1[feature_id], factor);
                // std::cout<<__FILE__<<":"<<__LINE__<<std::endl;
...
public:
    ...
    vector<vector<int64_t>> extract_answer(stringstream &reduced_poly_cipher_stream)
    {
        ...
        if (batch_size == poly_guest->half_poly_mod_degree) {
            auto reduced_poly_cipher = load_cipher_vector(
                poly_guest->get_context(), reduced_poly_cipher_stream, feature_num);
            ...
            auto reduced_poly_cipher = load_cipher_vector(
                poly_guest->get_context(), reduced_poly_cipher_stream, feature_num / factor);
            ...
            for (size_t feature_id = 0; feature_id < feature_num; feature_id++) {
                ...
                for (size_t i = 0; i < 2 * batch_size; i++) {
                    decomposed_plain_poly[feature_id][i] =
                        plain_poly[feature_id / factor][i * factor + feature_id % factor];
                }
            }
...
bool test_feature_correctness(
    const vector<vector<int64_t>> &data,
    const vector<int64_t> &new_ids,
    const vector<vector<int64_t>> &result0,
    const vector<vector<int64_t>> &result1,
    int64_t prime_num)
{
    ...
    size_t intersection_size)
{
    ...
    for (size_t feature_id = 0; feature_id < feature_num; feature_id++) {
        for (size_t i = 0; i < intersection_size; i++) {
            correctness &=
                (data.at(feature_id % data.size()).at(new_ids.at(i)) ==
                 (result0.at(feature_id).at(i) + result1.at(feature_id).at(i)) % prime_num);
        }
    }
    return correctness;
}

bool test_label_correctness(
    const vector<bool> &client_labels,
    int start,
    const vector<bool> &label_shares0,
    const vector<bool> &label_shares1)
{
    size_t sum0 = 0;
