...
public:
    PSItoShareServer(
        const vector<string> plaintext_ids,
        const vector<vector<int64_t>> &data,
        size_t client_size,
        size_t feature_num,
        bool lazy = false,
        bool use_out_mem = false)
        : data(data), feature_num(feature_num), lazy(lazy)
    {
        ec_player = make_shared<ECPlayer0>();
        ec_player->init(plaintext_ids);
        if (!lazy) {
            batch_pir_to_share_server =
                make_shared<BatchPirToShareServer>(data, client_size, feature_num, use_out_mem);
        }
        if (with_comm) {
    ...
    {
        batch_pir_to_share_server =
            make_shared<BatchPirToShareServer>(data, batch_size, feature_num);
    }
    // step 6
...
public:
    PSItoShareClient(
        size_t host_log_n_data, size_t feature_num, size_t batch_size, bool lazy = false)
        : host_log_n_data(host_log_n_data), feature_num(feature_num), batch_size(batch_size),
          lazy(lazy)
    {
        ec_player = make_shared<ECPlayer1>();
        if (!lazy) {
            batch_pir_to_share_client =
                make_shared<BatchPirToShareClient>(host_log_n_data, feature_num, batch_size);
        }
        if (with_comm) {
            recv_socket = coproto::asioConnect(recv_address, true);
            send_socket = coproto::asioConnect(send_address, false);
    ...
    std::ostream *out)
{
    ...
    *out << "Performance: " << std::endl;
    if (player == player_server) {
        ...
        *out << "get_newid: Gen psi_response time:" << psi_response_time << " ms " << std::endl;
        *out << "batch pir to share: Gen response time: " << response_time << " ms " << std::endl;
        auto server_online_time = psi_response_time + response_time;

        *out << "server_online_time=" << server_online_time << " ms " << std::endl;

        *out << "get_newid response size: " << psi_response.byte_size() / 1024.0 << " KBytes"
             << std::endl;
    ...
    if (player == player_client) {
        ...
        *out << "get_newid: Gen psi_query time:" << psi_query_time << " ms " << std::endl;
        *out << "get_newid: Gen psi_client_response time:" << psi_client_response_time << " ms "
             << std::endl;
        *out << "batch pir to share: Gen query time: " << query_time << " ms " << std::endl;
        *out << "batch pir to share: Extract answer time: " << extract_time << " ms " << std::endl;
        auto client_online_time =
            psi_query_time + psi_client_response_time + query_time + extract_time;
        *out << "client_online_time=" << client_online_time << " ms " << std::endl;

        *out << "get_newid query size: " << psi_query.byte_size() / 1024.0 << " KBytes"
             << std::endl;
        *out << "get new_id client response size: "
             << intersection_lastids.size() * sizeof(int32_t) / 1024.0 << " KBytes" << std::endl;
...
void psi_to_share_2party_socketopt(
    bool mode_switch,
    size_t host_log_n_data,
    size_t batch_size,
    size_t feature_num,
    ...
    *out << "Performance: " << std::endl;
    if (player == player_server) {
        ...
        *out << "comm. from server to client: "
        ...
        //      << (psi_response.byte_size() + response.str().size()) / 1024.0 << " KBytes"
        //      << std::endl;
        *out << "communication size (send + recv): "
             << (psi_response.byte_size() + response.str().size() + psi_query.byte_size() +
                 intersection_lastids.size() * sizeof(int32_t) + query.byte_size()) /
                    1024.0
             << " KBytes" << std::endl;
    ...
    std::ostream *out)
{
    ...
    *out << "------------------------------------" << std::endl;
    ...
    // if (player_server==player_client)
    // {
    //  bool feature_correctness =test_feature_correctness(data, new_ids, result0, result1,
    //  pss_client->get_prime_num(), intersection_size);
    //  *out<<"feature_correctness="<<feature_correctness<<std::endl;
    // bool label_correctness = test_label_correctness(client_labels, start, label_shares0,
    // label_shares1); *out<<"label_correctness="<<label_correctness<<std::endl;
    // }
}

void psi_to_share_2party_socketopt_fromfile(
    bool mode_switch, string file_name, size_t player, string out_file, std::ostream *out)
{
    //-----------数据准备--------- --------------
    size_t feature_num;
    ...
    if (player == player_server) {
        ...
        // auto pss_server = PSItoShareServer(server_ids, data, batch_size, feature_num);
        pss_server =
            make_shared<PSItoShareServer>(server_ids, data, batch_size, feature_num, false, true);
        server_init_time = timer.elapsed();
        std::cout << __FILE__ << ":" << __LINE__ << endl;
    }
