...
inline long reverse(long x, size_t bits_num)
{ // NOLINT
    bitset<64> bx(x), rbx;
...
void real_ifft(size_t log_da, const zz_pX &a, const FFTPrimeInfo &info, zz_pX &ifft_result)
{ // NOLINT
...
void real_ifft(
    ...
    std::vector<int64_t> &ifft_result)
{ // NOLINT
    // zz_pX new_fft_result;
    size_t da = (1 << log_da);
...
// void find_row_col_basis(const vec_zz_p &mat, size_t rows_num, vector<zz_pX> &row_keeper,
// vector<zz_pX> & col_keeper)
// {
...
//     vector<zz_pX> row_killer(rows_num), col_killer(cols_num); //, row_keeper(length),
//     col_keeper(length); vector<vec_zz_p> row_vecs(rows_num), col_vecs(cols_num);
...
void find_row_col_basis(
    const vec_zz_p &mat, size_t cols_num, vector<zz_pX> &row_keeper, vector<zz_pX> &col_keeper)
{
    size_t mat_size = mat.length();
    ...
    row_keeper.resize(rows_num);
    col_keeper.resize(cols_num);
    vector<zz_pX> row_killer(rows_num),
        col_killer(cols_num); //, row_keeper(length), col_keeper(length);
...
void find_row_col_basis(
    const vector<int64_t> &mat,
    size_t col_num,
    int64_t prime_num,
    vector<vector<uint64_t>> &row_keeper,
    vector<vector<uint64_t>> &col_keeper)
{
    // void find_row_col_basis(const vector<int64_t> &mat, size_t length, vector<vector<uint64_t>>
    // &row_keeper,  vector<vector<uint64_t>> & col_keeper)
    // {
    zz_p::init(prime_num);
    vec_zz_p ntl_mat;
...
public:
    PolyComputer(int64_t prime_num, int64_t log_degree)
        : // NOLINT
          q(prime_num), log_da(log_degree)
    {
        zz_p::init(q);
        // std::cout << "p=" << zz_p::modulus() << std::endl;
        if (!IsFFTPrime(q, w))
            LogicError("invalid prime");
    ...
    PolyComputer(int64_t prime_num, int64_t self_defined_w, int64_t log_degree)
        : // NOLINT
          q(prime_num), w(self_defined_w), log_da(log_degree)
    {
    ...
    void fft_interpolate(const vector<int64_t> &data, vector<int64_t> &coeff)
    { // NOLINT
    ...
    int check_fft_interpolate(const vector<int64_t> &data, const vector<int64_t> &coeff)
    {
        zz_pX poly;
        ...
        for (int64_t i = 0; i < 10; i++) {
            ...
            std::cout << "i, data[i], f(xi^i)=" << i << "," << data[i] << "," << y << std::endl;
            if (y != data[i]) {
    ...
    vector<int64_t> eval_poly_val(
        const vector<int64_t> &x, const vector<int64_t> &coeff, bool from_ids = true)
    {
        zz_pX poly;
    ...
    vector<vector<int64_t>> eval_batch_poly_val(
        const vector<int64_t> &x, const vector<vector<int64_t>> &coeff_mat, bool from_ids = true)
    {
        // zz_pX poly;
        // size_t len = coeff.size();
        // poly.SetLength(len);
    ...
    int check_poly_value(
        const vector<int64_t> &new_ids, const vector<int64_t> &coeff, const vector<int64_t> &data)
    {
        zz_pX poly;
        size_t len = coeff.size();
        poly.SetLength(len);
        ...
        for (int64_t i = 0; i < new_ids.size(); i++) {
            ...
            std::cout << "new_id, data, f(xi^new_id)=" << new_ids[i] << "," << data[i] << "," << y
                      << std::endl;
            if (y != data[i]) {
    ...
    void get_xi_powers(const vector<int64_t> &new_ids, vector<int64_t> &xi_power_new_ids)
    { // NOLINT
    ...
    void get_chara_poly(const vector<int64_t> &new_ids, zz_pX &chara_poly)
    { // NOLINT
        ...
        for (int i = 0; i < K; i++) {
            ...
            for (size_t j = 0; j < degree; j++) {
                if (j <= deg(tmp_x_powers_mod_chara_poly)) {
                    x_powers_mod_chara_poly[i][j] = rep(tmp_x_powers_mod_chara_poly[j]);
                } else {
    ...
    { // NOLINT
        ...
        get_chara_poly(new_ids, chara_poly);
        // std::cout << "chara_poly=" << chara_poly << std::endl;
        get_x_powers_mod_chara_poly(chara_poly, degree, K, x_powers_mod_chara_poly);
    }
    ...
    void get_chara_Xpowers_mod_new_ids(
        ...
        const size_t degree,
        const size_t K,
        std::vector<std::vector<int64_t>> &chara_x_powers_mod_chara_poly)
    { // NOLINT
        zz_pX chara_poly;
        get_chara_poly(new_ids, chara_poly);
        // std::cout << "chara_poly=" << chara_poly << std::endl;
        get_x_powers_mod_chara_poly(chara_poly, degree, K, chara_x_powers_mod_chara_poly);
