...
namespace ipcl_encrypt {
    ...
    inline void serialize_bignum(std::ostream &os, const BigNumber &big_num)
    {
        ...
        os.write(reinterpret_cast<const char *>(vec.data()), len * sizeof(uint32_t));
    ...
    inline void deserialize_basetext(std::istream &is, std::vector<BigNumber> &m_texts)
    {
        int len;
        is.read(reinterpret_cast<char *>(&len), sizeof(int));
        m_texts.resize(len);
    ...
    inline void serialize_ciphertext(std::ostream &os, const ipcl::CipherText &text)
    {
        serialize_basetext(os, text);
    }

    inline void deserialize_ciphertext(
        std::istream &is, const ipcl::PublicKey &pk, ipcl::CipherText &text)
    {
        std::vector<BigNumber> m_texts;
        deserialize_basetext(is, m_texts);
    ...
    inline void serialize_pub_key(std::ostream &os, const ipcl::PublicKey &pub_key)
    {
