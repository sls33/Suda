    ...
    std::ostream *out)
{
    ...
    if (player == player_server) {
        ...
        pss_server =
            make_shared<PSItoSharePaillierServer>(server_ids, data, batch_size, feature_num);
    ...
    if (player == player_client) {
        ...
        pss_client =
            make_shared<PSItoSharePaillierClient>(host_log_n_data, feature_num, batch_size);
