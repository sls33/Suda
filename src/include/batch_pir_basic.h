...
std::vector<string> ciphertext_to_string(const std::vector<seal::Ciphertext> &c)
{
...
void string_to_ciphertext(
    const SEALContext &context, const std::vector<string> &str_cipher, std::vector<Ciphertext> &c)
{ // NOLINT
...
void string_to_ciphertext(const SEALContext &context, const string &str_cipher, Ciphertext &c)
{ // NOLINT
    stringstream tmp_cipher_stream;
...
public:
        ...
        : host_log_n_data(host_log_n_data)
    {
        ...
        poly_computer = make_shared<PolyComputer>(prime_num, self_defined_w, host_log_n_data);
        /* Init BGV */
        ...
        if (type == 0) // pir
        {
            ...
            //     CoeffModulus::Create(poly_mod_degree, {48, 30, 24}));    //batch_pir
            //     最后噪声预算为10
            // parms.set_coeff_modulus(
            //     CoeffModulus::Create(poly_mod_degree, {43, 43, 43}));    //batch_pir
            //     最后噪声预算为10
            //             parms.set_coeff_modulus(
            // CoeffModulus::Create(poly_mod_degree, {54, 54, 54, 54}));
            parms.set_coeff_modulus(CoeffModulus::Create(poly_mod_degree, { 42, 42, 42, 42 }));
        ...
        if (init_key) {
            ...
            // std::vector<uint32_t> galois_eles = {1+2, 1+4, 1+8, 1+16, 1+32, 1+64, 1+128, 1+256,
...
public:
    ...
    PolyProcessorHost(size_t host_log_n_data, size_t batch_size, size_t type = 1)
        : batch_size(batch_size), PolyProcessor(0, type, host_log_n_data)
    { // NOLINT
    ...
    vector<Plaintext> encode(
        const vector<int64_t> &data, size_t batch_size, bool mod_switch_to_next = false)
    {
        vector<int64_t> coeff;
        ...
        for (size_t i = 0; i < coeff[0].size(); i++) {
            for (size_t j = 0; j < factor; j++) {
                ...
                if (i % batch_size == batch_size - 1 & j == factor - 1) {
                    piece_f = Plaintext(v_piece_f);
                    if (i > batch_size) // 常数项用来相加， 加法中BGV明文不可以是NTT form,
                                        // 密文必须要是NTT form
                    {
                        evaluator->transform_to_ntt_inplace(piece_f, context->first_parms_id());
                    }
    ...
    { // NOLINT
        ...
        if (chara_and_Xpowers_mod_chara.size() != pieces_f.size()) {
            std::cout << "chara_and_Xpowers_mod_chara.size()=" << chara_and_Xpowers_mod_chara.size()
                      << std::endl;
            std::cout << "pieces_f.size()=" << pieces_f.size() << std::endl;
            throw std::runtime_error("Check error: xpowers_mod_chara.size()!=pieces_f.size()");
        }
        //  std::cout<<__FILE__<<":"<<__LINE__<<std::endl;
        ...
        for (int64_t i = 0; i < pieces_f.size(); i++) {
            if (i == 0) {
                ...
                evaluator->multiply_plain(
                    chara_and_Xpowers_mod_chara[i], random_plaintext, reduced_cipher);
                //  std::cout<<__FILE__<<":"<<__LINE__<<std::endl;
            } else {
                evaluator->multiply_plain(
                    chara_and_Xpowers_mod_chara[i], pieces_f[i], tmp_reduced_cipher);
        ...
        if (parms.scheme() == scheme_type::bfv) {
            evaluator->transform_from_ntt_inplace(
                reduced_cipher); // 常数项是非NTT形式，bfv下不允许相加
        }
    ...
    void set_chara_and_Xpowers_mod_chara(
        const std::vector<string> &chara_and_Xpowers_mod_chara_str, bool mod_switch_to_next)
    {
        string_to_ciphertext(
            *context, chara_and_Xpowers_mod_chara_str, chara_and_Xpowers_mod_chara);
        if (mod_switch_to_next) {
            for (int i = 0; i < chara_and_Xpowers_mod_chara.size(); i++) {
                // evaluator->mod_switch_to_inplace(cipher_xpowers_mod_chara[i],
    ...
    void set_chara_and_Xpowers_mod_chara(
        stringstream &chara_and_Xpowers_mod_chara_str, bool mod_switch_to_next, size_t factor = 1)
    {
        // string_to_ciphertext(*context, chara_and_Xpowers_mod_chara_str,
        // chara_and_Xpowers_mod_chara);
        size_t vector_size = chara_and_Xpowers_mod_chara.size();
        chara_and_Xpowers_mod_chara =
            load_cipher_vector(*context, chara_and_Xpowers_mod_chara_str, vector_size);
        ...
        stringstream &chara_and_Xpowers_mod_chara_str, bool mod_switch_to_next, size_t factor = 1)
    {
        ...
        if (factor == 1) {
            ...
            auto chara_and_Xpowers_mod_chara_tmp =
                load_cipher_vector(*context, chara_and_Xpowers_mod_chara_str, vector_size / factor);
            // std::cout<<"chara_and_Xpowers_mod_chara_tmp.size()="<<chara_and_Xpowers_mod_chara_tmp.size()<<std::endl;
            chara_and_Xpowers_mod_chara = unzip_vec(
                *evaluator,
                chara_and_Xpowers_mod_chara_tmp,
                galois_keys,
                prime_num,
                poly_mod_degree,
                factor);
    ...
    void set_evaluate_basis(
        const std::vector<string> &row_keepers_str, const std::vector<string> &col_keepers_str)
    {
        ...
        for (size_t i = 0; i < batch_size; i++) {
            evaluator->multiply(
                row_keepers[i / col_num], col_keepers[i % col_num], evaluate_basis[i]);
    ...
    //         void set_evaluate_basis(stringstream & row_keepers_stream, stringstream &
    //         col_keepers_stream)
    //         {
    //             // vector<Ciphertext> row_keepers, col_keepers;
    //             // string_to_ciphertext(*context, row_keepers_str, row_keepers);
    ...
    //             vector<Ciphertext> row_keepers = load_cipher_vector(*context, row_keepers_stream,
    //             col_num); vector<Ciphertext> col_keepers = load_cipher_vector(*context,
    //             col_keepers_stream, col_num);

    //             std::cout<<"col_num="<<col_num<<std::endl;
    //             for (size_t i=0; i<batch_size; i++)
    //             {
    //                 evaluator->multiply(row_keepers[i/col_num], col_keepers[i%col_num],
    //                 evaluate_basis[i]); evaluator->mod_switch_to_next_inplace(evaluate_basis[i]);
    // #ifndef USE_NTT_IN_EVALUATION
    //                 evaluator->transform_from_ntt_inplace(evaluate_basis[i]);
    // #endif
    //             }
    ...
    vector<int64_t> poly_cipher_toShare(
        const Ciphertext &poly_cipher, Ciphertext &poly_cipher_share1)
    { // return share0 with size bath_size   (no poly_share0 with size 2*bath_size-1 )
        ...
        //  std::cout<<__FILE__<<":"<<__LINE__<<std::endl;
        //    std::cout<<"evaluate_basis[0]
        //    is_ntt_form<<"<<evaluate_basis[0].is_ntt_form()<<std::endl;
        //    std::cout<<"share0_plain_tmp
        //    is_ntt_form<<"<<share0_plain_tmp.is_ntt_form()<<std::endl;
    ...
    vector<vector<int64_t>> poly_cipher_toShare(
        const Ciphertext &poly_cipher, Ciphertext &poly_cipher_share1, size_t factor)
    { // return share0 with size bath_size   (no poly_share0 with size 2*bath_size-1 )
...
public:
    size_t half_poly_mod_degree = 4096;
    PolyProcessorGuest(size_t host_log_n_data, size_t batch_size, size_t type = 1)
        : batch_size(batch_size), PolyProcessor(1, type, host_log_n_data)
    { // NOLINT
    ...
    vector<string> gen_chara_Xpowers_cipher_polys(const vector<int64_t> &new_ids)
    {
        ...
        poly_computer->get_chara_Xpowers_mod_new_ids(
            new_ids, batch_size, K, x_powers_mod_chara_poly);
        ...
        if (batch_size == half_poly_mod_degree) {
            for (int k = 0; k < K; k++) {
                ...
                x_powers_mod_chara_poly_cipher[k] =
                    ciphertext_to_string(encryptor->encrypt_symmetric(plain_x_power_kn));
            }
    ...
    stringstream gen_chara_Xpowers_cipher_polys_stream(const vector<int64_t> &new_ids)
    {
        ...
        if (batch_size == half_poly_mod_degree) {
            ...
            for (int k = 0; k < K; k++) {
                ...
                // x_powers_mod_chara_poly_cipher[k] =
                encryptor->encrypt_symmetric(plain_x_power_kn)
                    .save(x_powers_mod_chara_poly_cipher_stream);
            }
        } else if (half_poly_mod_degree % batch_size == 0) {
            size_t factor = half_poly_mod_degree / batch_size;
            assert(factor * batch_size == half_poly_mod_degree);
...
#else
            auto plain_x_power_kns =
                interleave(x_powers_mod_chara_poly, (batch_size + 1) * factor, factor, prime_num);
            assert(plain_x_power_kns.size() == K / factor);
            for (size_t i = 0; i < plain_x_power_kns.size(); i++) {
                encryptor->encrypt_symmetric(plain_x_power_kns[i])
                    .save(x_powers_mod_chara_poly_cipher_stream);
            }
    ...
    // void gen_row_col_basis(const vector<int64_t> &new_ids, vector<string>& row_keepers_str,
    ...
    //         {
    //             row_keepers_str[i] =
    //             ciphertext_to_string(encryptor->encrypt_symmetric(Plaintext(row_keepers[i])));
    //             col_keepers_str[i] =
    //             ciphertext_to_string(encryptor->encrypt_symmetric(Plaintext(col_keepers[i])));
    //         }
    //     }
    //     else
    //     {
    ...
    //             row_keepers_str[i] =
    //             ciphertext_to_string(encryptor->encrypt_symmetric(prow_keeper));
    //             col_keepers_str[i] =
    //             ciphertext_to_string(encryptor->encrypt_symmetric(pcol_keeper));
    //         }
    //     }

    // }

    void gen_row_col_basis(
    ...
    vector<int64_t> decrypt_poly(const Ciphertext &c)
    {
        ...
        cout << "    + Noise budget in c: " << decryptor->invariant_noise_budget(c) << " bits"
             << endl;
    ...
    vector<int64_t> eveal_poly_value(
        const vector<int64_t> &revealed_poly,
        const vector<int64_t> &index_ids,
        bool from_ids = true)
    {
        // std::cout<<"shared_poly_coeff.size()="<<revealed_poly.size()<<std::endl;

        vector<int64_t> eval_res = poly_computer->eval_poly_val(index_ids, revealed_poly, from_ids);
    ...
    vector<vector<int64_t>> eval_batch_poly_value(
        const vector<vector<int64_t>> &polys,
        const vector<int64_t> &index_ids,
        bool from_ids = true)
    {
        ...
        auto eval_res = poly_computer->eval_batch_poly_val(index_ids, rem_polys, from_ids);
        return eval_res;
    }

    //-------------测试用----------------------------------------
    vector<int64_t> reveal_poly_value(
        const vector<int64_t> &shared_poly_coeff0,
        ...
        const vector<int64_t> &index_ids)
    {
        ...
        for (size_t i = 0; i < revealed_poly_size; i++) {
            revealed_poly[i] = (shared_poly_coeff0[i] + shared_poly_coeff1[i]) % prime_num;
        }

        vector<int64_t> eval_res = poly_computer->eval_poly_val(index_ids, revealed_poly);
        // for (size_t i = 0; i < eval_res.size(); i++) {
        //     eval_res[i] = eval_res[i] > prime_num / 2 ? eval_res[i] - prime_num
        //                                               : eval_res[i];
        // }
