...
int multiply_plain_normal_simple(
    SEALContext context, Ciphertext &encrypted, const Plaintext &plain, MemoryPoolHandle pool)
{
...
int multiply_plain_normal_simple(
    const std::vector<Modulus> &coeff_modulus,
    const Ciphertext &encrypted,
    const uint64_t plain,
    Ciphertext &encrypted1,
    MemoryPoolHandle pool)
...
// int multiply_plain_normal_simple2(const std::vector<Modulus> &coeff_modulus, const Ciphertext
// &encrypted, const size_t encrypted_size, const uint64_t plain, Ciphertext &encrypted1)
// {
...
std::vector<seal::Ciphertext> load_cipher_vector(
    SEALContext context, std::stringstream &vc_stream, size_t vector_size)
{
...
std::vector<Ciphertext> unzip(
    const Evaluator &evaluator,
    const std::vector<Ciphertext> &vcin,
    const GaloisKeys &galois_keys,
    uint64_t plain_module,
    size_t poly_mod_degree,
    size_t start_sparsity)
{
...
std::vector<Ciphertext> unzip_vec(
    const Evaluator &evaluator,
    const std::vector<Ciphertext> &vcin,
    const GaloisKeys &galois_keys,
    uint64_t plain_module,
    ...
    size_t end_sparsity)
{
    ...
    for (Ciphertext c : vcin) {
        auto vcout1 =
            unzip(evaluator, c, galois_keys, plain_module, poly_mod_degree, 1, end_sparsity);
