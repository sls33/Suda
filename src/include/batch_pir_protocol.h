...
void batch_pir_from_nttform_modswitch(
    bool isHost,
    bool isGuest,
...
void batch_pir_from_nttform_type0(
    bool isHost,
    bool isGuest,
    std::vector<std::vector<int64_t>> &data,
    vector<int64_t> &new_ids,
...
private:
    ...
    shared_ptr<PolyProcessorHost> poly_host = nullptr;
    size_t
        fake_feature_num; // 应对内存不足，计算仍按照feature_num进行，存储就只存贮fake_feature_num多维特征
    size_t factor = 1;

public:
    BatchPirServer(const vector<vector<int64_t>> &data, size_t batch_size, size_t feature_num)
        : feature_num(feature_num)
    {
        ...
        if (batch_size == poly_host->half_poly_mod_degree) {
            ...
            for (size_t i = 0; i < feature_num / factor; i++) {
                auto data_tobe_encode = vector<vector<int64_t>>(
...
bool test_correctness(
    ...
    const vector<vector<int64_t>> &result)
{
