...
int read_cols_server(
    string file_name, std::vector<std::string> &server_ids, std::vector<std::vector<int64_t>> &data)
{
...
int read_rows(
    string file_name, std::vector<int64_t> &server_ids, std::vector<std::vector<int64_t>> &data)
{
...
int read_1row(string file_name, std::vector<T> &server_ids, std::vector<int64_t> &data)
{
    ...
    std::cout << "data.size()=" << data.size() << std::endl;
    // std::cout<<"data[0].size()="<<data[0].size()<<std::endl;
    std::cout << "1.0*data.size()/server_ids.size()=" << 1.0 * data.size() / server_ids.size()
              << std::endl;
    return 0;
}
...
inline int save_csv_transpose(
    std::vector<std::vector<int64_t>> data,
    std::vector<bool> label_share,
