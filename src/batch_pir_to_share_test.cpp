...
void batch_pir_to_share_test(bool modswitch)
{
    ...
    //-----------Server initialize-----------------------
    timer.reset();
    batch_pir_to_share_from_nttform_modswitch(
        isHost,
        isGuest,
        data,
        new_ids,
...
void batch_pir_to_share_test1(bool mode_switch, size_t batch_size, std::ostream *out)
{
    //-----------数据准备--------- --------------
    ...
    *out << "Query size: " << query.byte_size() / 1024.0 << " KBytes" << std::endl;
...
int main(int argc, char *argv[])
{
    ...
    batch_pir_to_share_test1(modswitch, 1024, out);
}